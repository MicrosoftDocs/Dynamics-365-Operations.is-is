---
title: Nýjar eða uppfærðar verkefnaleiðbeiningar (Maí 2016)
description: Þetta málefni sýnir nýjar eða uppfærðar verkefnaleiðbeiningar sem voru gefnar út fyrir Microsoft (ágúst 2016 - öll tungumál) Unified Library for Microsoft Dynamics AX. Engin ný Safn hafist handa var losuð.
author: josaw1
manager: AnnBe
ms.date: 06/20/2017
ms.topic: article
ms.prod: ''
ms.service: dynamics-ax-platform
ms.technology: ''
audience: Application User, IT Pro
ms.reviewer: sericks
ms.custom: 104233
ms.assetid: 2a2ffa1e-5a15-4e2a-b18c-c4d890d38a50
ms.search.region: global
ms.author: josaw
ms.search.validFrom: 2016-05-31
ms.dyn365.ops.version: Platform update 1
ms.openlocfilehash: d17c038081b5c9b94337549334c2c61421dc01ff
ms.sourcegitcommit: f5e31c34640add6d40308ac1365cc0ee60e60e24
ms.translationtype: HT
ms.contentlocale: is-IS
ms.lasthandoff: 12/08/2020
ms.locfileid: "4694966"
---
# <a name="new-or-updated-task-guides-may-2016"></a>Nýjar eða uppfærðar verkefnaleiðbeiningar (Maí 2016)

[!include [banner](../includes/banner.md)]

Þetta málefni sýnir nýjar eða uppfærðar verkefnaleiðbeiningar sem voru gefnar út fyrir Microsoft (ágúst 2016 - öll tungumál) Unified Library for Microsoft Dynamics AX. Engin ný Safn hafist handa var losuð. 

## <a name="task-guides-in-the-may-2016---all-languages-apqc-unified-library-for-microsoft-dynamics-ax"></a>Verkleiðbeiningar í (maí 2016 - öll tungumál) Unified Library for Microsoft Dynamics AX

(Maí 2016) APQC Unified Library inniheldur verkleiðbeiningar sem er ætlaðar eru fyrir Hjálp með Microsoft Dynamics AX maí 2016 uppfærslu og Microsoft Dynamics AX 2016 febrúarútgáfu. Staðbundnar útgáfur af verkefnaleiðbeiningum sem voru gefnar út í (febrúar 2016) APQC Unified Library eru tiltækar í þessu safni. Nánari upplýsingar um hvernig tengja á hjálparkerfið við verkefnaleiðbeiningar er að finna í [Hjálparkerfi](help-overview.md).

> [!NOTE]
> Stjarna (\*) við titil línunnar fyrir viðskiptaferlavinnslu (BPM) sýnir verkefnaleiðbeiningar fyrir viðskiptaferli sem var stofnað fyrir Dynamics AX maí uppfærslu 2016 og er ekki studdur í eldri útgáfu af Microsoft Dynamics AX.

### <a name="countryregion-functionality"></a>Virkni lands/svæðis

| BPM-lína                                                                 | Land/svæði                    |
|--------------------------------------------------------------------------|-----------------------------------|
| Setja upp rafræna OIOUBL-reikningsfærslu                                       | Danmörk                           |
| EUR-00002 Mynda ESB intrastat-skattaskýrslu                           | Evrópu                            |
| EUR-00002 Flytja færslur í intrastat                         | Evrópu                            |
| FR-00004 Uppfæra greiðslumáta viðskiptavinar                            | Evrópu                            |
| Koma greiðsluháttum á fót                                    | Frakkland                            |
| FR-00003 NAF-kóðar og siret-númer                                     | Frakkland                            |
| FR-00004 Samþykkja og bóka víxil                          | Frakkland                            |
| Skilgreina færibreytur accelerated afskriftir og bókunarreglur       | Japan                             |
| Skilgreina færibreytur sameinuðum reikningi og uppsetningu fyrir lánardrottna | Japan                             |
| Stofna skjal hraðaðra afskriftir og færa inn gögn um notkun         | Japan                             |
| Stofna FSE-flokka og fjárskapandi einingar                               | Japan                             |
| Losa eigna með lækkun færslu                            | Japan                             |
| MY-00004 Stjórna debetnótu lánardrottins og kreditnótu fyrir GST                | Malasíu Singapúr og Taíland |

### <a name="data-access"></a>Gagnaheimildir

| BPM-lína                                       | Land/svæði |
|------------------------------------------------|----------------|
| Skilgreina fjárhagslega samnýtingu gagna milli fyrirtækja | Altæk         |

### <a name="electronic-reporting"></a>Rafræn skýrslugerð

| BPM-lína                                                                   | Land/svæði |
|----------------------------------------------------------------------------|----------------|
| Rafræn skýrslugerð skilgreina áfangastað                                                  | Altæk         |
| Rafræn skýrslugerð Hanna skilgreiningu til að mynda skýrslur á OPENXML-sniði         | Altæk         |
| Rafræn skýrslugerð Mynda rafræn skjöl fyrir greiðslur með skilgreiningu á sniði | Altæk         |
| Rafræn skýrslugerð Flytja inn skilgreiningu úr Lifecycle Services                          | Altæk         |
| \*Rafræn skýrslugerð Velja þjónustuveitu                                               | Altæk         |
| Rafræn skýrslugerð Uppfærsðu snið með því að taka upp nýja grunnútgáfu sniðs      | Altæk         |
| Rafræn skýrslugerð Hlaða upp skilgreiningu í Lifecycle Services                          | Altæk         |

### <a name="human-capital-management"></a>Mannauðsstjórnun

| BPM-lína                                    | Land/svæði |
|---------------------------------------------|----------------|
| Dreifa spurningalistum með röðun | Altæk         |
| Setja upp stöður                            | Altæk         |

### <a name="inventory-management"></a>Birgðastjórnun

| BPM-lína                                           | Land/svæði |
|----------------------------------------------------|----------------|
| Skilgreina sjálfgefnar pöntunarstillingar fyrir vörur.            | Altæk         |
| Setja upp forsendur fyrir ósamkvæmistýringu. | Altæk         |

### <a name="manufacturing"></a>Framleiðsla

| BPM-lína                                           | Land/svæði |
|----------------------------------------------------|----------------|
| Afrita fyrirliggjandi formúlu aukaafurðir          | Altæk         |
| Stofna Kanban fyrir BOM-línutilvik                | Altæk         |
| Stofna staðgengils kanban-reglu                   | Altæk         |
| Stofna kanban-reglu úttektar                    | Altæk         |
| Framkvæma kanban-ferlisvinnslur                        | Altæk         |
| Áætla framleiðslupöntun.                        | Altæk         |
| Raða framleiðsluverkum fyrir vinnsluframleiðslu. | Altæk         |
| Stöðuuppfærsla kanbans                               | Altæk         |

### <a name="master-planning"></a>Aðaláætlanagerð

| BPM-lína                      | Land/svæði |
|-------------------------------|----------------|
| Stofna áætlun fyrir svæði      | Altæk         |
| Stofna svæði áætlun        | Altæk         |
| Fylgjast með aðaláætlunarkeyrslu | Altæk         |

### <a name="organization-administration"></a>Fyrirtækisstjórnun

| BPM-lína                                       | Land/svæði |
|------------------------------------------------|----------------|
| Stofna lögaðila                          | Altæk         |
| Stofna stigveldi fyrirtækisins.             | Altæk         |
| Setja upp númeraröð á einstaklingsgrundvelli | Altæk         |

### <a name="procurement-and-sourcing"></a>Innkaup og aðföng

| BPM-lína                                        | Land/svæði |
|-------------------------------------------------|----------------|
| Samþykkja lánardrottinn fyrir ákveðnar afurðir           | Altæk         |
| Stofna innkaupapöntun                         | Altæk         |
| Stofna beiðni um tilboð                  | Altæk         |
| Færa inn og bera saman kauptilboð vegna beiðni um TILBOÐ og veita aðilum samninga. | Altæk         |

### <a name="product-information-and-configuration"></a>skilgreining og vöruupplýsingar

| BPM-lína                                                 | Land/svæði |
|----------------------------------------------------------|----------------|
| Stofna nýja afurð                                     | Altæk         |
| Stofna skilgreiningar byggðar á vídd                    | Altæk         |
| Stofna fyrirframskilgreind afurðarafbrigði                       | Altæk         |
| Stofna söluverð valskilyrði                    | Altæk         |
| Færa inn pantanir fyrir stillanlegar afurðir                   | Altæk         |
| Setja upp verðlagningu byggðri á eigind fyrir stillanlegar afurðir | Altæk         |

### <a name="retail-and-commerce"></a>Smásala og viðskipti

| BPM-lína                                                           | Land/svæði |
|--------------------------------------------------------------------|----------------|
| Úthluta verkum                                                       | Altæk         |
| Skilgreina og keyra vinnsluna ítrekað til að uppfæra birgða                | Altæk         |
| Dreifing dreifingarstjórnunar vörur frá móttöku vöruhúss til að geyma              | Altæk         |
| Færa vörur frá dreifingarmiðstöð til að geyma með dreifing frá lager | Altæk         |

### <a name="security"></a>Öryggi

| BPM-lína                       | Land/svæði |
|--------------------------------|----------------|
| Skipa notendum í öryggishlutverk | Altæk         |

### <a name="sales-tax"></a>Virðisaukaskattur

| BPM-lína                                           | Land/svæði |
|----------------------------------------------------|----------------|
| Reikna og leiðrétta vsk á reikningi lánardrottins | Altæk         |
| Stofna greiðslu á VSK                         | Altæk         |
| Stofna vsk-færslur á skjölum         | Altæk         |
| Setja upp fjárhagsbókunarflokka fyrir virðisaukaskatt         | Altæk         |
| Setja upp skattyfirvöld                       | Altæk         |
| Setja upp VSK-kóða                             | Altæk         |
| Setja upp VSK-flokka og VSK-flokka fyrir vörur  | Altæk         |
| Setja upp skýrslugerðarkóða virðisaukaskatts                   | Altæk         |
| Setja upp virðisaukaskatttímabil                | Altæk         |
| Setja upp staðgreiðsluskatt                             | Altæk         |
| Skoða bókaðar virðisaukaskattsfærslur                 | Altæk         |

### <a name="security"></a>Öryggi

| BPM-lína                                                | Land/svæði |
|---------------------------------------------------------|----------------|
| Nýir notendur stofnaðir                                        | Altæk         |
| Bera kennsl og leysa úr árekstrum fyrir aðskilnað á skyldum | Altæk         |
| Setja upp aðskilnaður á skyldum                            | Altæk         |

### <a name="warehouse-and-transportation"></a>Vöruhúsakerfi og flutningar

|                                     BPM-lína                    | Land/svæði |
|-----------------------------------------------------------------|----------------|
| Skilgreina fyrir Úrvinnsla bylgna                                       | Altæk         |
| \*Skrá sem lokið í staðsetningu sem ekki er plötustýrð | Altæk         |
| Setja upp lágmarks-hámarks áfyllingarvinnslu                          | Altæk         |
| Setja upp gámun                                         | Altæk         |
| Setja upp handvirkt pökkun.                                           | Altæk         |
| Setja upp flutningsskorður fyrir vöru                   | Altæk         |
| \*Setja upp vinnureglur vöruhúss                                | Altæk         |

## <a name="additional-resources"></a>Frekari upplýsingar

[Nýjar verkefnaleiðbeiningar (febrúar 2016)](new-task-guides-available-february-2016.md)

[Nýjar eða uppfærðar verkefnaleiðbeiningar (ágúst 2016)](new-updated-task-guides-available-august-2016.md)

[Nýjar eða uppfærðar verkefnaleiðbeiningar (nóvember 2016)](new-task-guides-november-2016.md)


[!INCLUDE[footer-include](../../../includes/footer-banner.md)]