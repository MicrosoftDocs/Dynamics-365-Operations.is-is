...
## <a name="check-for-initial-synchronization-errors-in-a-finance-and-operations-app"></a>Athugaðu hvort fyrstu samstillingarvillur séu í fjárhags- og rekstrarforriti
...
1. Í fjármála- og rekstrarappinu skaltu eyða **Auðkenni aðaltengiliðs** og **InvoiceVendor AccountNumber** dálka úr vörpuninni og vistaðu síðan vörpunina.
...
1. Í fjármála- og rekstrarappinu skaltu eyða **ContactPersonID** og **InvoiceAccount** dálkar frá **Viðskiptavinir V3 (reikningar)** kortlagningu og vistaðu síðan kortlagninguna.
...
6. Keyrið aftur upphaflega samstillingu fyrir vörpunina **Viðskiptavinir V3 (Lyklar)**. Vegna þess að slökkt er á breytingarakningu eru gögnin fyrir **InvoiceAccount** og **ContactPersonId** verður samstillt úr fjármála- og rekstrarappinu til Dataverse.
7. Til að samstilla gögnin fyrir **InvoiceAccount** og **ContactPersonId** frá Dataverse við fjármála- og rekstrarappið verður þú að nota gagnasamþættingarverkefni.

    1. Í Power Apps, búa til gagnasamþættingarverkefni milli **Sölureikningur** og **fjármála- og rekstrarforrit.Viðskiptavinir V3** borðum. Gagnaáttin verður að vera frá Dataverse í fjármála- og rekstrarappið. Þar sem **InvoiceAccount** er ný eigind í tvöföldum skrifum gætirðu viljað sleppa upphaflegri samstillingu fyrir hana. Nánari upplýsingar er að finna í [Sameina gögn í Dataverse](/power-platform/admin/data-integrator).
    ...
    2. Bættu við fyrirtækjaviðmiðunum í síuna á Dataverse hlið, þannig að aðeins línur sem passa við síuskilyrðin verða uppfærðar í fjármála- og rekstrarappinu. Til að bæta við síu skal velja síuhnappinn. Því næst, í svarglugganum **Breyta fyrirspurn** er hægt að bæta við síufyrirspurn á borð við **\_msdyn\_fyrirtæki\_gildi jafngildir „\<guid\>“**.
...
8. Í fjármála- og rekstrarappinu skaltu kveikja aftur á breytingarakningu fyrir **Viðskiptavinir V3** borð.
