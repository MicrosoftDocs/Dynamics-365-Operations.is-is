...
Eftirfarandi þrjú Data Factory sniðmát eru til staðar. Þeir hjálpa til við að samræma gögnin frá báðum Finance and Operations öpp og öpp fyrir þátttöku viðskiptavina.
...
| Skrárnafn | Notkun |
|---|---|
| FONewParty.csv | Þessi skrá hjálpar til við að búa til nýja **Partí** skrár inni í Finance and Operations app. |
| ImportFONewPostalAddressLocation.csv | Þessi skrá hjálpar til við að búa til nýja **Póstfang Staðsetning** skrár í Finance and Operations app. |
| ImportFONewPartyPostalAddress.csv | Þessi skrá hjálpar til við að búa til nýja **Póstfang aðila** skrár í Finance and Operations app. |
| ImportFONewPostalAddress.csv | Þessi skrá hjálpar til við að búa til nýja **Póstfang** skrár í Finance and Operations app. |
| ImportFONewElectronicAddress.csv | Þessi skrá hjálpar til við að búa til nýja **Rafræn heimilisfang** skrár í Finance and Operations app. |
...
> [!IMPORTANT]
> Það eru sérstakar leiðbeiningar ef þú ætlar að keyra sniðmát fyrir flokkspóstfang og rafræn heimilisfang aðila. Þú verður að keyra sniðmát aðila fyrst, síðan sniðmát póstfangs aðila og síðan sniðmát fyrir rafrænt heimilisfang aðila.
...
+ **Skyndimynd:** Taktu mynd af báðum Finance and Operations öpp og öpp fyrir þátttöku viðskiptavina. Þú getur síðan notað skyndimyndirnar til að endurheimta fyrra ástand ef þú þarft.
...
1. Skráðu þig inn í forrit fyrir þátttöku viðskiptavina og farðu á **Stillingar** \> **Sérstillingar**. Síðan, á **Almennt** flipanum, stilltu tímabeltisstillingu fyrir kerfisstjórareikninginn. Tímabelti verður að vera í samræmdum alheimstíma (UTC) til að uppfæra „gild frá“ og „gild til“ dagsetningar póstfanga frá kl.Finance and Operations öpp.
...
2. Í Data Factory, á **Stjórna** flipi, undir **Alþjóðlegar breytur**, búðu til eftirfarandi alþjóðlega færibreytu.
    ...
    |---|---|---|---|
    | 1 | PostalAddressIdPrefix | strengur | Þessi færibreyta bætir raðnúmeri við nýstofnað póstföng sem forskeyti. Vertu viss um að gefa upp streng sem stangast ekki á við póstföng í Finance and Operations öpp og öpp fyrir þátttöku viðskiptavina. Til dæmis, notaðu **ADF-PAD-**. |
...
1. Í Data Factory, á **Stjórna** flipi, undir **Alþjóðlegar breytur**, búðu til eftirfarandi alþjóðlegu færibreytur.
    ...
    | Númer | Heiti | Gerð | Gildi |
    |---|---|---|---|
    | 1 | IsFOSource | ból | Þessi færibreyta ákvarðar hvaða aðalkerfisföngum er skipt út ef árekstrar koma upp. Ef gildið er **satt**, aðalheimilisföngin í Finance and Operations öpp munu skipta um aðalheimilisföng í öppum fyrir þátttöku viðskiptavina. Ef gildið er **rangt**, munu aðalheimilisföngin í forritum fyrir þátttöku viðskiptavina koma í stað aðalheimilisföngin í Finance and Operations öpp. |
    | 2 | ElectronicAddressIdPrefix | strengur | Þessi færibreyta bætir raðnúmeri við nýstofnuð rafræn heimilisföng sem forskeyti. Vertu viss um að gefa upp streng sem stangast ekki á við rafræn heimilisföng í Finance and Operations öpp og öpp fyrir þátttöku viðskiptavina. Til dæmis, notaðu **ADF-EAD-**. |
...
1. Hættu eftirfarandi **Reikningur**, **samband**, og **Seljandi** tvöfalt skrifa kort sem nota Finance and Operations app:
...
2. Gakktu úr skugga um að kortin séu fjarlægð af **msdy_dualwriteruntimeconfig** borð í Dataverse.
3. Setjið upp [Lausn tvöfaldrar skráningar á aðila og altækri aðsetursbók](https://aka.ms/dual-write-gab) úr AppSource.
4. Í Finance and Operations app, hlaupa **Upphafleg samstilling** fyrir eftirfarandi töflur ef þær innihalda gögn:
...
9. Í gagnaverksmiðjunni skaltu keyra sniðmát fyrir póstfang aðila og rafræn heimilisfang aðila, hvert á eftir öðru.
    ...
    + Póstfangssniðmát aðila uppfærir allar póstfangsskrár í samskiptaforriti viðskiptavina og tengir þær við samsvarandi **Reikningur**, **samband**, og **Seljandi** skrár. Það býr einnig til þrjár .csv skrár: ImportFONewPostalAddressLocation.csv, ImportFONewPartyPostalAddress.csv og ImportFONewPostalAddress.csv.
    + Sniðmát fyrir rafræn heimilisfang aðila setur öll rafræn heimilisföng í viðskiptaforritinu upp og tengir þau við samsvarandi **Reikningur**, **samband**, og **Seljandi** skrár. Það býr einnig til eina .csv skrá: ImportFONewElectronicAddress.csv.

    ![Að keyra sniðmát fyrir póstfang aðila og rafræn heimilisfang aðila.](media/ADF-7.png)

10. Til að uppfæra Finance and Operations app með þessum gögnum verður þú að umbreyta .csv skránum í Excel vinnubók og [flytja það inn í Finance and Operations app](/data-entities/data-import-export-job). Að öðrum kosti, ef CSV innflutningurinn virkar fyrir þig, geturðu flutt inn .csv skrárnar beint. Þetta skref gæti tekið nokkrar klukkustundir að ljúka, allt eftir hljóðstyrknum.
...
2. Skref 7-1 til 7-9 sækja gögn úr báðum Finance and Operations app og viðskiptavinaþátttökuforritið og settu þessi gögn á svið til uppfærslu.
3. Skref 8 til 9 bera saman flokksnúmerið fyrir **Reikningur**, **samband**, og **Seljandi** skrár milli Finance and Operations appið og forritið fyrir þátttöku viðskiptavina. Öllum færslum sem hafa ekki aðilanúmer er sleppt.
4. Skref 10 býr til tvær .csv skrár fyrir aðilaskrárnar sem þarf að búa til í forritinu fyrir þátttöku viðskiptavina og Finance and Operations app.
...
1. Skref 1-1 til 1-10 sækja gögn úr báðum Finance and Operations app og viðskiptavinaþátttökuforritið og settu þessi gögn á svið til uppfærslu.
2. Skref 2 afnormalizes póstfangsgögnin í Finance and Operations app með því að sameina póstfangið og póstfang aðila.
3. Skref 3 afritar og sameinar reiknings-, tengiliða- og heimilisfangsgögn frá viðskiptavinaforritinu.
4. Skref 4 býr til .csv skrár fyrir Finance and Operations app til að búa til ný heimilisfangsgögn sem eru byggð á reikningi, tengiliðum og heimilisfangi söluaðila.
5. Skref 5-1 býr til .csv skrár fyrir þátttökuforrit viðskiptavina til að búa til öll heimilisfangsgögn, byggð á báðum Finance and Operations appið og forritið fyrir þátttöku viðskiptavina.
...
### <a name="steps-in-the-party-electronic-address-template"></a>Skref í sniðmát fyrir rafrænt heimilisfang aðila

1. Skref 1-1 til 1-5 sækja gögn úr báðum Finance and Operations app og viðskiptavinaþátttökuforritið og settu þessi gögn á svið til uppfærslu.
2. Skref 2 sameinar rafræn heimilisföng í þátttökuforriti viðskiptavina frá reiknings-, tengiliða- og söluaðilum.
3. Þrep 3 sameinar aðal rafræn heimilisfangsgögn frá þátttökuforriti viðskiptavina og Finance and Operations app.
4. Skref 4 býr til .csv skrár.

    - Búðu til ný rafræn heimilisfangsgögn fyrir Finance and Operations app, byggt á reikningi, tengiliðum og heimilisfangi söluaðila.
    - Búðu til ný rafræn heimilisfangsgögn fyrir þátttökuforrit viðskiptavina, byggt á netfangi, reikningi, tengiliðum og heimilisfangi söluaðila í Finance and Operations app.
