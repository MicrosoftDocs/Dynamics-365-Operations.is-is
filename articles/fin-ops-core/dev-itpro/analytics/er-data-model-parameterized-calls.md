...
description: Þessi grein útskýrir hvernig á að útfæra símtöl með færibreytum í rafrænum skýrslugerð (ER) gagnalíkönum.
author: kfend
ms.date: 03/14/2022
ms.topic: article
ms.prod: ''
...
Þegar þú keyrir ER snið er hver sniðþáttur keyrður, byrjaður á rótarsniði. Alltaf þegar sniðþáttur sem er keyrður inniheldur bindingu við a [gagnagjafa](general-electronic-reporting.md#configuring-data-model-mappings-for-outgoing-documents), gagnagjafinn er keyrður til að afhenda væntanleg gögn og nota þau til að fylla út sniðþáttinn. Þegar gagnagjafi á *Fyrirmynd* gerð er kölluð, er viðeigandi líkanskortlagning kallað til að draga gögn út úr forritinu, byggt á stillingum líkanakortlagningar.

Áður var ekki hægt að breyta þessum líkanakortlagningarsímtölum til að gera þau háð rökfræði sniðsins sem er keyrt. Aðeins eftirfarandi gagnaflæði var stutt.
...
Hins vegar, í útgáfu 10.0.15 og nýrri, er hægt að stilla reiti gagnalíkana sem aðeins er hægt að kalla þegar gildi stilltu færibreytanna eru tilgreind. Þegar slíkur reitur er stilltur og kallaður til úr sniðhluti verður að gefa upp nauðsynlegar færibreytur í sniðbindingu sem rök fyrir símtalinu. Í þessum tilvikum er hægt að tilgreina gildi röksemda út frá sniðssértækum gildum. Þess vegna getur þú notað **dynamic runtime parametrisation** fyrir gagnalíkanasímtöl til að styðja við eftirfarandi gagnaflæði.
...
Nýja virknin gerir þér kleift að stilla símtalið í hvaða gagnalíkansvið sem er [*Met*](er-formula-supported-data-types-composite.md#record) eða [*Met listi*](er-formula-supported-data-types-composite.md#record-list) tegund. Eftirfarandi gagnagerðir eru studdar fyrir færibreytur gagnalíkanareits:
...
Þú getur tilgreint hverja færibreytu gagnalíkanareitar sem hægt er að gefa upp röksemdafærsluna sem eitt gildi skilgreindrar gagnagerðar og [*lista*](er-formula-supported-data-types-composite.md#record-list) slíkra verðmæta.
...
Eftirfarandi skref útskýra hvernig notandi í hlutverki kerfisstjóra eða þróunaraðila rafrænna skýrslna getur hannað ER lausn sem inniheldur gagnalíkan, líkanavörpun og snið ER hluti sem eru stilltir til að kalla á líkanavörpun frá sniði með því að útvega rök fyrir símtalinu, en gildi þess hefur verið reiknað út á keyrslutíma með því að nota formúlu hlaupasniðsins. 
...
Þú verður að búa til nýja ER-stillingu sem inniheldur nauðsynlegan gagnalíkanahluta. Þetta gagnalíkan verður notað sem gagnagjafi síðar, þegar þú hannar ER-snið til að búa til endurskoðunarskýrslu.

Fylgdu þessum skrefum til að flytja inn nauðsynlegt gagnalíkan úr XML skrá sem er útveguð af Microsoft.

1. Sækja [Dæmi um endurskoðunarmodel.version.1.xml](https://download.microsoft.com/download/7/1/9/719b0132-fed7-4c73-8afa-90cac29c2fee/Sample-audit-model.version.1.xml) skrá og vistaðu hana á heimatölvunni þinni.
...
![Uppbygging ER-gagnalíkans á síðunni Gagnalíkönhönnuður.](./media/er-data-model-parameterized-calls-model1.png)
...
Sem notandi í hlutverki þróunaraðila rafrænna skýrslna verður þú að búa til nýja ER-stillingu sem inniheldur líkanavörpun fyrir sýnishorn endurskoðunargagnalíkans. Þessi hluti útfærir stillt gagnalíkan fyrir Microsoft Dynamics 365 Finance og er sérstaklega fyrir það app. Skilgreina verður íhlut líkanavörpunar til að tilgreina hugbúnaðarhluti sem þarf að nota til að fylla út skilgreint gagnalíkan með forritsgögnum við keyrslu. Til að klára þetta verkefni verður þú að skilja hvernig gagnaskipulag skattaviðskiptalénsins er útfært í Finance.

Fylgdu þessum skrefum til að flytja inn nauðsynlega líkanavörpun úr XML skrá sem er útveguð af Microsoft.

1. Sækja [Dæmi um úttektarlíkan mapping.version.1.1.xml](https://download.microsoft.com/download/c/0/3/c03a4673-b1b1-4ef8-96d0-bf96518be6e0/Sample-audit-model-mapping.version.1.1.xml) skrá og vistaðu hana á heimatölvunni þinni.
2. Farðu í **Fyrirtækisstjórnun** \> **Vinnusvæði** \> **Rafræn skýrslugerð**.
3. Á vinnusvæðinu **Rafræn skýrslugerð** skal velja **Skilgreiningar skýrslugerðar**.
4. Á **Stillingar** síðu, á aðgerðarrúðunni, veldu **Skipti** \> **Hlaða úr XML skrá**.
...
Eftirfarandi mynd sýnir breytanlegu útgáfu af stilltu líkanavörpunni á **Módelkortahönnuður** síðu.
...
- Í líkanavörpun þinni skaltu stilla bindinguna fyrir nýja gagnalíkanareitinn með því að nota tiltæk töflutengsl til að fá aðgang að viðeigandi skrá yfir`DirPartyTable` umsóknartöflu og sæktu gildið á`Name` sviði frá því.

Þó að þessi nálgun virki gæti hún valdið afköstum í SQL gagnagrunninum, vegna þess að`TaxTrans` er færslutaflan og getur því innihaldið mikið magn af færslum. Í þessu tilviki er fjöldi símtala til`DirPartyTable` verður að jafna fjölda skráa í`TaxTrans` töflu sem getur valdið frammistöðuvandamálum.
...
- Í líkanavörpuninni skaltu bæta við nýjum gagnagjafa sem mun sameina allar skrár yfir töflur sem eru notaðar í töflusamböndum til að fá aðgang að viðkomandi skrá yfir`DirPartyTable` umsóknartafla, frá og með`TaxTrans` borð.

Þó að þessi aðferð muni virka gæti hún valdið minnisnotkunarvandamálum. Jafnvel þegar ný [GANGA TIL](er-join-data-sources.md) gagnagjafi er keyrt sem ein SQL beiðni til umsóknargagnagrunnsins til að koma í veg fyrir afköst gagnagrunnsvandamála, niðurstaðan verður að vera sótt í minni forritaþjónsins. Vegna þess að fjöldi skráa og fjöldi reita í þeim skrám verður nokkuð mikill gæti þessi aðferð valdið mjög mikilli minnisnotkun. Undantekning utan minnistíma gæti jafnvel verið hent.

Þú getur innleitt breytingarnar þegar keyrandi snið safnar, í minni, einstökum auðkenniskóðum viðskiptavina og lánardrottna fyrir allar skattfærslur sem verða kynntar á myndaðri skýrslu. Vegna þess að aðeins ætti að halda einstökum kóða, verður lokasett kóða ekki nógu stórt til að hafa áhrif á minnisnotkun. Kóðasettið verður síðan sent til líkankortlagningarinnar sem rök fyrir öðru símtali gagnagjafans *Fyrirmynd* tegund. Byggt á því símtali mun líkankortlagningin keyra nýjan ER gagnagjafa sem gerir eina SQL beiðni til forritagagnagrunnsins til að sækja, frá`DirPartyTable` töflu, skrár aðeins fyrir þá aðila sem eru með kóða í meðfylgjandi kóðasetti.
...
3. Á **Útgáfur** Flýtiflipi, veldu útgáfu **2** sem hefur stöðu á **Drög**.
...
9. Í **Færibreytur** valmynd, fylgdu þessum skrefum:
    ...
    5. Veldu **Allt í lagi** til að klára að slá inn breytur.
...
9. Fylgdu þessum skrefum til að bæta við nýjum gagnagjafa til að biðja um`DirPartyTable` skrár, byggðar á tilgreindum lista yfir auðkenniskóða skráa:
    ...
    11. Í **Færibreytur** valmynd, fylgdu þessum skrefum:
        ...
        > [!NOTE]
        > Þú hefur bara stillt þennan reiknaða reit þannig að hann samþykki, á keyrslutíma, röksemdafærslu einnar færibreytu sem er stilltur sem skráarlisti sem hefur eina`Value` sviði á *Int64* tegund.
...
10. Fylgdu þessum skrefum til að binda nýja gagnagjafann við nýja gagnalíkansviðið, þannig að gagnalíkanið sé notað til að afhjúpa nöfn aðila:
    ...
    7. Á **Formúluhönnuður** síðu, í **Uppruni gagna** rúðu, stækka **Gögn \> DirParty**, og veldu **Nafn**.
...
8. Fylgdu þessum skrefum til að bæta við nýjum sniðeiningu til að safna skráaauðkenniskóðum aðila sem færslur eru sýndar á mynduðum skýrslum.

    1. Í sniði uppbyggingu tré, veldu **Report.Row.Trans** röð þáttur.
    2. Veljið **Bæta við**.
    3. Í **Bæta við** valmynd, veldu **Uppruni gagna \> Atriði**.
    4. Í **Eiginleikar íhluta** valmynd, í **Nafn** reit, slá inn **kt**.
    5. Í **Gagnategund** reit, veldu **Int64**.
    6. Veldu **Í lagi**.

    > [!NOTE]
    > A **Uppruni gagna \> Atriði** eining er hægt að nota til að framkvæma innri útreikninga og gagnaumbreytingu aðeins á umfangi hlaupandi sniðs. Þess vegna, með því að bæta við þessari sniðeiningu, breytirðu ekki innihaldi myndaðs skjals.

9. Fylgdu þessum skrefum til að bæta við nýjum sniðsþáttum til að slá inn nöfn aðila í útbúnar skýrslur:
...
12. Fylgdu þessum skrefum til að bæta við nýjum bindingum til að slá inn nöfn aðila á myndaðar skýrslur:
    ...
    1. Í sniði uppbyggingu tré, veldu **Report.Party** röð þáttur.
    2. Veljið **Breyta formúlu**.
    3. Á **Formúluhönnuður** síðu, sláðu inn tjáninguna `model.Party(PartyIds.Result)`.
    4. Veljið **Vista** og lokið síðunni **Formúluhönnuður**.
    5. Í sniði uppbyggingu tré, veldu **Report.Party.Name** röð þáttur.
