---
title: Keyra Launavinnsla
description: Þetta efnisatriði útskýrir hvernig á að leysa mál þar sem viðskiptamaður reynir að keyra launavinnslu í Microsoft Dynamics 365 for Talent en kemst ekkert áleiðis.
author: andreabichsel
manager: AnnBe
ms.date: 11/02/2018
ms.topic: article
ms.prod: ''
ms.service: dynamics-365-talent
ms.technology: ''
audience: Application User
ms.reviewer: anbichse
ms.search.scope: Talent
ms.custom: ''
ms.assetid: ''
ms.search.region: Global
ms.author: anbichse
ms.search.validFrom: 2018-11-02
ms.dyn365.ops.version: Talent
ms.openlocfilehash: 865baedd66eb6d87d7c2ddef1b552cc831345842
ms.sourcegitcommit: 2b890cd7a801055ab0ca24398efc8e4e777d4d8c
ms.translationtype: HT
ms.contentlocale: is-IS
ms.lasthandoff: 05/07/2019
ms.locfileid: "1518307"
---
# <a name="run-the-compensation-process"></a>Keyra launavinnslu

[!include [banner](includes/banner.md)]

**Úthreyfing**

Viðskiptamaður reynir að keyra launavinnslu í Microsoft Dynamics 365 for Talent en kemst ekkert áleiðis.

**Orsök**

Viðskiptamaður þarf fylgiskjölin.

**Upplausn**

Hér eru fylgiskjöl fyrir launin:

- Uppsetning:

    - [Launafyrirkomulag](https://docs.microsoft.com/en-us/dynamics365/unified-operations/talent/compensation-plans)
    - [Launafyrirkomulag fastra launa stofnað](https://docs.microsoft.com/en-us/dynamics365/unified-operations/talent/create-fixed-compensation-plans)
    - [Launafyrirkomulag breytilegra launa](https://docs.microsoft.com/en-us/dynamics365/unified-operations/talent/create-variable-compensation-plans)

- Vinnsla:

    - [Launaútreikningur](https://docs.microsoft.com/en-us/dynamics365/unified-operations/talent/process-compensation)
