---
title: Mannauður – heimasíða
description: Dynamics 365 Human Resources veitir vinnuafls innsýn sem þú þarft til að byggja upp upplifun starfsmanna á mörgum sviðum.
author: twheeloc
ms.date: 09/03/2022
ms.topic: article
ms.prod: ''
ms.technology: ''
ms.search.form: DefaultDashboard
audience: Application User
ms.custom:
- "260624"
- intro-internal
ms.assetid: ''
ms.search.region: Global
ms.author: twheeloc
ms.search.validFrom: 2020-02-03
ms.dyn365.ops.version: Human Resources
ms.openlocfilehash: 111070e3ce05e0f60f02259f73be56536fc017e7
ms.sourcegitcommit: 27ce4fc706100b626b81c3a1023238acd872e76c
ms.translationtype: MT
ms.contentlocale: is-IS
ms.lasthandoff: 10/20/2022
ms.locfileid: "9702041"
---
# <a name="human-resources-home-page"></a>Mannauður – heimasíða

Þessi grein veitir lista yfir hjálparefni og önnur úrræði í Microsoft Dynamics 365 Mannauður. 

Veldu eiginleikasvæði til að fræðast frekar um viðkomandi eiginleika.

- [Sjálfsafgreiðsla starfsmanns](hr-employee-manager-self-service-overview.md)
- [Starfsmannastjórnun](hr-personnel-departments-jobs-positions.md)
- [Verkstjórnun](hr-task-mgmt.md)
- [Leyfi og fjarvistir](hr-leave-and-absence-overview.md)
- [Fríðindi](hr-benefits-manage-program.md)
- [Fríðindastjórnun](hr-benefits-management-overview.md)
- [Launastjórnun](hr-compensation-overview.md)
- [Þróun starfsmanns](hr-develop-performance-management-overview.md)
- [Nám](hr-learning-courses.md)
- [Verkflæðiskerfi](../fin-ops-core/fin-ops/organization-administration/overview-workflow-system.md?toc=/dynamics365/human-resources/toc.json)

Human Resources tengir fólk og rekstrargögn til að hjálpa þér að hámarka kostnað vinnuafls og sjá um starfsmenn þína.

## <a name="dynamics-365-finance"></a>Dynamics 365 Finance

Fyrir upplýsingar um Dynamics 365 Finance, sjá [Heimasíða fjármála](../finance/index.md).

## <a name="additional-resources"></a>Frekari upplýsingar

### <a name="elearning-courses"></a>Rafræn námskeið

Netnámskeið og þjálfun má finna í [Dynamics 365 Human Resources](/training/browse/?products=dynamics-human-resources&expanded=dynamics-365).

### <a name="videos"></a>Myndbönd

Kynnið ykkur kennslumyndböndin sem eru aðgengileg á [Microsoft Dynamics 365 YouTube rásinni](https://www.youtube.com/channel/UCJGCg4rB3QSs8y_1FquelBQ).

[!INCLUDE[footer-include](../includes/footer-banner.md)]
