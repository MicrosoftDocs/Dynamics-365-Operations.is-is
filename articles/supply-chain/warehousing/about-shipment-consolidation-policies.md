---
title: Yfirlit yfir samstæðureglur sendingar
description: Í þessari grein er að finna yfirlit yfir virkni sem býður upp á sveigjanlega skilgreiningu samstæðureglna sendinga.
author: Mirzaab
ms.date: 05/12/2020
ms.topic: article
ms.prod: ''
...
---
# <a name="shipment-consolidation-policies-overview"></a>Yfirlit yfir samstæðureglur sendingar
...
Þegar kveikt er á eiginleikanum *Samstæðureglur sendingar* fyrir kerfið verður stillingin **Sameina sendingar við losun í vöruhús** sem var áður tiltæk á uppsetningarsíðunni **Vöruhús** falin. Til að aðstoða við umbreytingu í nýja samstæðueiginleika sendingar er aðgerð á síðu **samstæðureglna sendingar** sem gerir þér kleift að stofna sjálfgefna reglu sem inniheldur sjálfkrafa gömlu stillingarnar fyrir núverandi vöruhús. Eftir að þessi sjálfgefna regla er stofnuð er stillingin **Taka sendingu með í samstæðu við losun í vörugeymslu** á uppsetningarsíðunni **Vöruhús** ekki lengur tekin til greina. Frekari upplýsingar er að finna í [Skilgreina samstæðureglur sendingar](configure-shipment-consolidation-policies.md).
...
| Eftirfarandi samstæðureitur skilgreinir einkvæma samsetningu gilda sem notuð er til að sameina sendingar fyrir *flutningslínu*. (Allir aðrir reitir eru hunsaðir.)<ul><li>Pöntunarnúmer (OrderNum)</li></ul> | Eftirfarandi samstæðureitir skilgreina einkvæma samsetningu gilda sem notuð er til að sameina sendingar fyrir *flutningslínu*. (Allir aðrir reitir eru hunsaðir.)<ul><li>Pöntunarnúmer (OrderNum)</li><li>Viðtakandi sendingar (DeliveryName)</li><li>Póstfang (DeliveryPostalAddress)</li><li>ISO-landskóði (CountryRegionISOCode)</li><li>Heimilisfang (Address)</li><li>Svæði (InventSiteId)</li><li>Vöruhús (InventLocationId)</li><li>Farmflytjandi (CarrierCode)</li><li>Flutningsþjónusta (CarrierServiceCode)</li><li>Afhendingarmáti (ModeCode) \*</li><li>Flutningsaðilaflokkur (CarrierGroupCode)</li><li>Afhendingarskilmálar (DlvTermId)</li></ul><p>Þessir reitir eru einu reitirnir sem eru í boði og frumstilltir þegar ný sending er stofnuð.</p><p>\* Athugaðu: *ModeCode* er **Sniðið** sem úthlutað er á **Farmflytjanda** sem valinn er fyrir flutningslínu (ekki **Afhendingarmáta** sem valinn er fyrir flutningslínu). Ef þú velur að hafa með *Afhendingarmáti (ModeCode)* á meðal sameiningarskilyrðis, þá mun kerfið aðeins sameina flutningslínur með sama gildinu fyrir **Snið** svo lengi sem **Farmflytjandi**, **Flutningsþjónusta** og **Afhendingarmáti** sé skilgreint fyrir línuna (óháð gildum þeirra). Að auki mun kerfið einnig sameina allar flutningslínur þar sem **Stilling** er auður.</p> |
| Eftirfarandi samstæðureitir skilgreina einkvæma samsetningu gilda sem notuð er til að sameina sendingar fyrir *sölulínu*. (Allir aðrir reitir eru hunsaðir.)<ul><li>Pöntunarnúmer (OrderNum)</li><li>Tilvísun viðskiptavinar (CustomerRef)</li><li>Innkaupabeiðni viðskiptavinar (CustomerReq)</li><li>Afhendingarskilmálar (DlvTermId)</li></ul> | Eftirfarandi samstæðureitir skilgreina einkvæma samsetningu gilda sem notuð er til að sameina sendingar fyrir *sölulínu*. (Allir aðrir reitir eru hunsaðir.)<ul><li>Pöntunarnúmer (OrderNum)</li><li>Reikningsnúmer (AccountNum)</li><li>Viðtakandi sendingar (DeliveryName)</li><li>Póstfang (DeliveryPostalAddress)</li><li>ISO-landskóði (CountryRegionISOCode)</li><li>Heimilisfang (Address)</li><li>Svæði (InventSiteId)</li><li>Vöruhús (InventLocationId)</li><li>Farmflytjandi (CarrierCode)</li><li>Flutningsþjónusta (CarrierServiceCode)</li><li>Afhendingarmáti (ModeCode) \*</li><li>Flutningsaðilaflokkur (CarrierGroupCode)</li><li>Kenni miðlara (BrokerCode)</li><li>Stefna (LoadDirection)</li><li>Afhendingarskilmálar (DlvTermId)</li><li>Tilvísun viðskiptavinar (CustomerRef)</li><li>Innkaupabeiðni viðskiptavinar (CustomerReq)</li></ul><p>Þessir reitir eru einu reitirnir sem eru í boði og frumstilltir þegar ný sending er stofnuð.</p><p>\* Athugaðu: *ModeCode* er **Sniðið** sem úthlutað er á **Farmflytjanda** sem valinn er fyrir sölulínu (ekki **Afhendingarmáta** sem valinn er fyrir sölulínu). Ef þú velur að hafa *Afhendingarmáti (ModeCode)* á meðal sameiningaskilyrðis, þá mun kerfið aðeins nota sölulínur sameiningar með sama gildinu fyrir **Snið** svo lengi sem **Farmflytjandi**, **Flutningsþjónusta** og **Afhendingarmáti** sé skilgreint fyrir línuna (óháð gildum þeirra). Að auki mun kerfið einnig sameina allar sölulínur þar sem **Stilling** er auður.</p> |
| Á ekki við | Eftirfarandi samstæðureitir eru áskildir fyrir *sölulína* og ekki er hægt að fjarlægja þá:<ul><li>Reikningsnúmer (AccountNum)</li><li>Viðtakandi sendingar (DeliveryName)</li><li>Póstfang (DeliveryPostalAddress)</li><li>Vöruhús (InventLocationId)</li></ul>Sjálfgefið er að þessum reitum sé úthlutað þegar ný regla er stofnuð. Ekki er hægt að fjarlægja þá. |
