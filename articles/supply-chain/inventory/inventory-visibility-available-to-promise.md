...
description: Þetta efnisatriði lýsir því hvernig á að tímasetja framtíðar breytingar á hendi og reikna út magn sem er tiltækt til að lofa (ATP).
author: yufeihuang
ms.date: 03/04/2022
ms.topic: article
ms.search.form: ''
...
The *ATP reiknaður mælikvarði* er fyrirfram skilgreind útreiknuð mælikvarði sem venjulega er notaður til að finna það magn sem er í boði núna. Summa samlagningarbreytimagnsins er framboðsmagnið og summan af frádráttarbreytimagninu er eftirspurnarmagnið.

Þú getur bætt við mörgum reiknuðum mælikvarða til að reikna ATP magn. Hins vegar ætti heildarfjöldi breytinga í öllum ATP-reiknuðum mælikvarða að vera færri en níu.
...
1. Stilltu eftirfarandi svæði:
    ...
    - **Dagskrá Tímabil** – Sláðu inn fjölda daga sem notendur geta skoðað og sent inn áætlaðar breytingar á hendi þegar valinn reiknaður mælikvarði er notaður. Notendur sem spyrjast fyrir um hlutabréfaupplýsingar munu fá tiltækt magn, áætlaðar breytingar á hendi og ATP fyrir hvern dag á þessu tímabili, frá og með núverandi dagsetningu. Veldu heiltölu á milli 1 og 7.
...
Til dæmis leggurðu inn pöntun fyrir 10 hjól og býst við að þau berist á morgun. Þess vegna sendir þú inn breytingaáætlun sem hefur innleiðingarmagnið 10 og er dagsett fyrir morgundaginn. Þegar pöntunin berst daginn eftir bætir þú hjólunum við á lagerinn þinn. Þú verður síðan að skuldbinda breytinguna á kerfinu þínu til að uppfæra raunverulegt magn á lager. Til að framfylgja breytingunni sendir þú inn breytingatilvik sem hefur 10 á heimleið. Þú afturkallar síðan áætlaða breytingu með því að senda inn breytingaáætlun fyrir hendi sem hefur magn á heimleið upp á -10.
...
- **Dagsetning** – Dagsetningin sem niðurstaðan á við.
...
- **ATP magn** – Lágmarks áætluð magn sem er í boði frá tilgreindri dagsetningu til loka áætlunartímabilsins. Þetta magn inniheldur allar áætlaðar magnleiðréttingar. Það er hámarksmagnið sem hægt er að lofa á núverandi degi fyrir afhendingu eða neyslu þann dag.
...
1. Á núverandi dagsetningu (1. febrúar 2022) sendir þú inn eftirfarandi áætlaðar magnbreytingar:
    ...
    - Krafa um 3 magn fyrir 6. febrúar 2022
...
| Slóð | Aðferð | Lýsing |
| --- | --- | --- |
| `/api/environment/{environmentId}/on-hand/changeschedule` | `POST` | Búðu til eina áætlaða breytingu á hendi. |
| `/api/environment/{environmentId}/on-hand/changeschedule/bulk` | `POST` | Búðu til margar áætlaðar breytingar á hendi. |
...
### <a name="submit-on-hand-change-schedules"></a>Sendu inn tímabundnar breytingaráætlanir

Skipulagsáætlanir eru gerðar með því að senda inn a`POST` beiðni til viðeigandi vefslóð birgðasýnileikaþjónustu (sjá [Sendu breytingaráætlanir, breyttu atburði og ATP fyrirspurnir í gegnum API](#api-urls) kafla). Þú getur líka sent inn magnbeiðnir.

Til að senda inn breytingaáætlun fyrir hendi verður meginmál beiðninnar að innihalda auðkenni fyrirtækis, vöruauðkenni, áætlaða dagsetningu og magn eftir dagsetningu. Áætluð dagsetning verður að vera á milli núverandi dagsetningar og loka núverandi áætlunartímabils.

#### <a name="example-request-body-that-contains-a-single-update"></a>Dæmi um beiðni meginmál sem inniheldur eina uppfærslu

Eftirfarandi dæmi sýnir beiðni meginmál sem inniheldur eina uppfærslu.
...
#### <a name="example-request-body-that-contains-multiple-bulk-updates"></a>Dæmi um meginmál beiðni sem inniheldur margar (magn) uppfærslur

Eftirfarandi dæmi sýnir meginmál beiðni sem inniheldur margar (magn) uppfærslur.
...
### <a name="submit-on-hand-change-events"></a>Sendu inn breytingaviðburði
...
Í beiðni þinni skaltu stilla`QueryATP` til *satt* ef þú vilt spyrjast fyrir um áætlaðar breytingar á hendi og ATP niðurstöður.
