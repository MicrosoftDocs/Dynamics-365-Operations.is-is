...
description: Þessi grein veitir upplýsingar um skýja- og jaðarkvarðaeiningar fyrir framleiðslu- og vöruhúsastjórnunarálag.
author: Mirzaab
ms.date: 04/22/2021
ms.topic: article
...
>
> Þegar einingarkvarðar fyrir ský og edge eru virkjaðir verður þú beðin(n) að staðfesta að þú skiljir að sum gögn sem tengjast skilgreiningu og úrvinnslu einingarkvarða í skýi og edge kunni að vera geymd í gagnamiðstöðvum sem eru staðsettar í Bandaríkjunum. Til að læra meira um gagnavinnslu fyrir skýja- og brúnkvarðaeiningar, sjáðu [Gagnavinnsla við stjórnun mælieininga](#data-processing-management) kafla síðar í þessari grein.
...
Einingarkvarðar stækka umhverfi Supply Chain Management miðstöðvarinnar með því að bæta við úthlutaðri úrvinnslugetu. Hægt er að keyra einingarkvarða í skýinu. Að öðrum kosti geta þeir keyrt á [brún](cloud-edge-edge-scale-units-lbd.md), á staðnum á staðnum þinni.
...
Hægt er að stilla umhverfi miðstöðvar og einingarkvarða skýs fyrir valið vinnuálag með því að nota [Stjórnandagátt einingarkvarða](https://sum.dynamics.com). Einnig er hægt að úthluta fleiri en einu vinnuálagi á hvern einingarkvarða. Fyrir upplýsingar um forsendur og takmarkanir fyrir skýjakvarðaeiningar í núverandi útgáfu, sjá [Forsendur og takmarkanir fyrir skýjakvarðaeiningar](#cloud-scale-unit-prerequisites) kafla síðar í þessari grein.
...
## <a name="considerations-before-you-enable-the-distributed-hybrid-topology-for-supply-chain-management"></a>Athugasemdir áður en þú virkjar dreifða blendinguna fyrir birgðakeðjustjórnun

Með því að virkja dreifða blendinguna, umbreytir þú Supply Chain Management skýumhverfinu þínu þannig að það virki sem miðstöð. Einnig er hægt að tengja frekari umhverfi við sem eru skilgreind sem einingarkvarðar í skýinu eða jaðrinum.
...
Gögnum sem eru flutt til og geymd í bandarískum gagnamiðstöðvum verður eytt í samræmi við reglur Microsoft um gagnavarðveislu. Persónuvernd þín skiptir Microsoft máli. Frekari upplýsingar má finna í [tilkynningu okkar um persónuvernd](https://go.microsoft.com/fwlink/?LinkId=521839).

## <a name="onboard-to-the-distributed-hybrid-topology-for-supply-chain-management"></a>Um borð í dreifða blendinguna fyrir birgðakeðjustjórnun

### <a name="try-out-the-distributed-hybrid-topology"></a>Prófaðu dreifða blendinga svæðisfræðina

Ferlið við að fara um borð í dreifða blendinguna hefur tvö stig. Á fyrsta stigi ættir þú [prófa](cloud-edge-try-out.md) lausnina og staðfestu aðlögun þína til að ganga úr skugga um að þær virki í dreifðri staðfræði sem inniheldur mælikvarðaeiningar. (Þú getur notað núverandi þróunarumhverfi til að gera löggildinguna.) Þú getur síðan haldið áfram á annað stig, þar sem þú eignast framleiðsluumhverfi.
...
> [!TIP]
> Til að bera kennsl á þá stærð sem best uppfyllir þarfir þínar skaltu vinna með innleiðingarfélaga þínum og Microsoft til að skilja þá mánaðarlegu viðskiptastærð sem þú þarfnast.
...
> [!NOTE]
> Viðbætur við mælieininga eru ekki tengdar við takmarkaðan fjölda notenda. Þau eru tiltæk fyrir hvaða notanda sem er í núverandi áskrift þinni (að því gefnu að kerfisstjórinn þinn hafi úthlutað þeim nauðsynlegum notendahlutverkum).
...
Eftir að þú hefur lokið við að skipuleggja hvernig þú ætlar að fara um borð í dreifða blendinguna fyrir birgðakeðjustjórnun muntu nota [Gátt Scal Unit Manager](https://aka.ms/SCMSUM) til að hefja inngönguferlið. Í gáttinni skal velja flipann **Leigjendur Dynamics 365**. Þessi flipi sýnir lista yfir leigjendur sem reikningurinn þinn er hluti af og þar sem þú ert eigandi eða stjórnandi umhverfis fyrir LCS-verk.
...
| Flyttu kvarðaeiningu yfir á miðstöðina | Tímasettu vinnuálag sem nú er í gangi á mælikvarðaeiningu til að flytja í miðstöðina. Þegar þetta ferli er keyrt mun samstilling gagna streyma og bæði miðstöðin og mælieiningin verða stillt til að breyta eignarhaldi vinnuálagsins.
